...
int g_iMenuSelected[MAXPLAYERS + 1];
int g_iRefireAtOnce[MAX_ENTITIES];
int g_iRefireCount[MAX_ENTITIES];
int g_iSelectedTrig;
int g_iTriggers[MAX_ENTITIES];

bool g_bLoaded;

float g_fRefireTime[MAX_ENTITIES];
float g_fTargetAng[MAX_ENTITIES];
float g_vTargetZone[MAX_ENTITIES][3];
...
public void OnAllPluginsLoaded() {
    if (!LibraryExists("l4d2_airstrike"))
...
public void OnPluginStart() {
    ...
    RegAdminCmd("sm_show_airstrike",  CmdShowAirstrikeById, ADMFLAG_ROOT, "Usage: sm_show_airstrike <index>", "", 0);
...
public Action CmdShowAirstrikeById(int client, int args) {
    char index[4];
...
void ShowMenuTargetList(int client, int index) {
    ...
    for (int i = 0; i < MAX_ENTITIES; i++) {
        if (index == 0) {
            ...
            } else if (IsValidEntRef(g_iTriggers[i])) {
...
void CreateTrigger(int index = -1, int client) {
    if (index == -1) {
        for (int i = 0; i < MAX_ENTITIES; i++) {
            if (g_vTargetZone[i][0] == 0.0 && g_vTargetZone[i][1] == 0.0 && g_vTargetZone[i][2] == 0.0 && !IsValidEntRef(g_iTriggers[i])) {
                index = i;
                break;
            }
        }
