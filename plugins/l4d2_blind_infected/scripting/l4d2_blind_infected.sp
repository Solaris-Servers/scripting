...
ArrayList g_BlockedEntitiesArray = null;
...
public void OnPluginStart() {
    L4D2Weapons_Init();
    g_BlockedEntitiesArray = new ArrayList(eArray_Size);
...
public Action Timer_EntCheck(Handle hTimer) {
    ...
    int iCurrentEnt[eArray_Size];
    int iEntity;
    int iSize = g_BlockedEntitiesArray.Length;
    for (int i = 0; i < iSize; i++) {
        g_BlockedEntitiesArray.GetArray(i, iCurrentEnt[0], sizeof(iCurrentEnt));
        iEntity = EntRefToEntIndex(iCurrentEnt[eEntRef]);
        if (iEntity != INVALID_ENT_REFERENCE && !iCurrentEnt[eHasBeenSeen] && IsVisibleToSurvivors(iEntity)) {
            GetEntPropString(iEntity, Prop_Data, "m_ModelName", sTmp, sizeof(sTmp));
            iCurrentEnt[eHasBeenSeen] = true;
            g_BlockedEntitiesArray.SetArray(i, iCurrentEnt[0], sizeof(iCurrentEnt));
        }
    }
        ...
        if (iWeapon) {
            for (int j = 0; j < sizeof(g_iIdsToBlock); j++) {
                if (iWeapon == g_iIdsToBlock[j]) {
                    ...
                    iBhTemp[eEntRef] = EntIndexToEntRef(i);
                    iBhTemp[eHasBeenSeen] = false;
                    g_BlockedEntitiesArray.PushArray(iBhTemp[0], sizeof(iBhTemp));
                    break;
                }
            }
        }
...
public Action OnTransmit(int iEntity, int iClient) {
    ...
    for (int i = 0; i < iSize; i++) {
        g_BlockedEntitiesArray.GetArray(i, iCurrentEnt[0], sizeof(iCurrentEnt));
        if (iEntity == EntRefToEntIndex(iCurrentEnt[eEntRef])) {
            return (iCurrentEnt[eHasBeenSeen]) ? Plugin_Continue : Plugin_Handled;
        }
    }
    return Plugin_Continue;
}
...
bool IsVisibleTo(int iClient, int iEntity) {
    float fAngles[3], fOrigin[3], fEnt[3], fLookAt[3];
    GetClientEyePosition(iClient, fOrigin);                     // get both player and zombie position
    GetEntPropVector(iEntity, Prop_Send, "m_vecOrigin", fEnt);  // get both player and zombie position
...
public bool TraceFilter(int iEntity, int iContentsMask) {
    // dont let WORLD, players, or invalid entities be hit
