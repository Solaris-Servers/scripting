...
NativeVote Data_CreateVote(NativeVotesType voteType, MenuAction actions) {
    GlobalForward handler     = CreateForward(ET_Single, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
    GlobalForward voteResults = CreateForward(ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Array, Param_Array, Param_Cell, Param_Array, Param_Array);
    KeyValues vote = new KeyValues("NativeVote");
...
bool Data_AddItem(KeyValues vote, const char[] info, const char[] display) {
    ...
    if (infoArray == null || displayArray == null || infoArray.Length >= Game_GetMaxItems() || displayArray.Length >= Game_GetMaxItems()) {
        return false;
    }
...
bool Data_InsertItem(KeyValues vote, int position, const char[] info, const char[] display) {
    ArrayList infoArray = view_as<ArrayList>(vote.GetNum(INFO, view_as<int>(INVALID_HANDLE)));
    ArrayList displayArray = view_as<ArrayList>(vote.GetNum(DISPLAY, view_as<int>(INVALID_HANDLE)));
    if (infoArray == null || displayArray == null || infoArray.Length >= Game_GetMaxItems() || displayArray.Length >= Game_GetMaxItems() || position >= infoArray.Length) {
        return false;
    }
...
bool Data_RemoveItem(KeyValues vote, int position) {
    ArrayList infoArray = view_as<ArrayList>(vote.GetNum(INFO, view_as<int>(INVALID_HANDLE)));
    ArrayList displayArray = view_as<ArrayList>(vote.GetNum(DISPLAY, view_as<int>(INVALID_HANDLE)));
    if (infoArray == null || displayArray == null || position >= infoArray.Length || position < 0) {
        return false;
    }
