...
// Forward
GlobalForward g_OverrideMaps;
StringMap g_MapOverrides;
bool g_OverrideNextCallVote[MAXPLAYERS + 1];

enum struct CallVoteForwards
{
    GlobalForward CallVote_Forward;
    GlobalForward CallVote_Vis;
}

enum struct CallVoteListData
{
    int CallVoteList_VoteType;
    bool CallVoteList_VoteEnabled;
}

CallVoteForwards g_CallVotes[NativeVotesOverride_Size];
int CallVoteListData_blocksize = 0;
...
public Plugin myinfo = {
    name        = "NativeVotes",
    author      = "Powerlord",
    description = "Voting API to use the game's native vote panels. Compatible with L4D, L4D2.",
    version     = "1.1.1",
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) {
    ...
    CreateNative("NativeVotes_Pass",                      Native_Pass);
...
{
    CallVoteListData temp;
    CallVoteListData_blocksize = sizeof(temp);

    LoadTranslations("core.phrases");
    LoadTranslations("nativevotes.phrases.txt");

    g_Cvar_VoteHintbox = CreateConVar(
    "nativevotes_progress_hintbox", "0", "Show current vote progress in a hint box",
    FCVAR_NONE, true, 0.0, true, 1.0);

    g_Cvar_VoteChat = CreateConVar(
    "nativevotes_progress_chat", "0", "Show current vote progress as chat messages",
    FCVAR_NONE, true, 0.0, true, 1.0);

    g_Cvar_VoteConsole = CreateConVar(
    "nativevotes_progress_console", "0", "Show current vote progress as console messages",
    FCVAR_NONE, true, 0.0, true, 1.0);

    g_Cvar_VoteClientConsole = CreateConVar(
    "nativevotes_progress_client_console", "0", "Show current vote progress as console messages to clients",
    FCVAR_NONE, true, 0.0, true, 1.0);

    g_Cvar_VoteDelay = CreateConVar(
    "nativevotes_vote_delay", "30", "Sets the recommended time in between public votes",
    FCVAR_NONE, true, 0.0);

    Game_InitializeCvars();
    g_Cvar_VoteDelay.AddChangeHook(OnVoteDelayChange);
...
public Action Command_CallVote(int client, const char[] command, int argc) {
    ...
    switch (argc) {
        ...
        case 0: {
            ArrayList hVoteTypes = new ArrayList(view_as<int>(CallVoteListData_blocksize)); // Stores arrays of CallVoteListData
            ...
            for (int i = 1; i < sizeof(g_CallVotes); i++) {
                if (GetForwardFunctionCount(g_CallVotes[i].CallVote_Forward) > 0) {
                    ...
                    int pos = FindVoteInArray(hVoteTypes, i);
                    if (pos > -1) {
                        hVoteTypes.GetArray(pos, voteType);
                        ...
                        #endif
                        voteType.CallVoteList_VoteType    = i;
                        voteType.CallVoteList_VoteEnabled = true;
        ...
        default: {
            ...
            int overrideType = Game_VoteStringToVoteOverride(voteCommand);
            char argument[PLATFORM_MAX_PATH];
            if (GetForwardFunctionCount(g_CallVotes[overrideType].CallVote_Forward) == 0) {

                if (g_MapOverrides != null && (overrideType == NativeVotesOverride_ChgLevel || overrideType == NativeVotesOverride_NextLevel)) {
...
stock int FindVoteInArray(ArrayList myArray, int value) {
...
stock bool IsVoteEnabled(ArrayList myArray, int value) {
    int pos = FindVoteInArray(myArray, value);
    if (pos > -1) {
        CallVoteListData voteType;
        myArray.GetArray(pos, voteType.CallVoteList_VoteType, sizeof(voteType.CallVoteList_VoteType));
...
void StartVoting() {
    if (g_hCurVote == null) {
        ThrowNativeError(SP_ERROR_NATIVE, "NativeVotes handle %x is invalid", g_hCurVote);
        return;
    }
    ...
    // If g_hCurVote was closed after initiator's vote
    // we abort execution of the code bellow to prevent spam error logs.
    if (g_hCurVote == null)
        return;

    // Kick targets automatically vote no if they're in the pool
    if (voteType == NativeVotesType_Kick || voteType == NativeVotesType_KickCheating || voteType == NativeVotesType_KickIdle || voteType == NativeVotesType_KickScamming) {
    ...
    if (g_bPassed) {
        for (int i = 1; i <= MaxClients; i++) {
            if (g_ClientVotes[i] == VOTE_PENDING) {
                g_hVotes.Set(NATIVEVOTES_VOTE_YES, g_hVotes.Get(NATIVEVOTES_VOTE_YES) + 1);
            } else if (g_ClientVotes[i] == NATIVEVOTES_VOTE_NO) {
                g_hVotes.Set(NATIVEVOTES_VOTE_NO,  g_hVotes.Get(NATIVEVOTES_VOTE_NO)  - 1);
                g_hVotes.Set(NATIVEVOTES_VOTE_YES, g_hVotes.Get(NATIVEVOTES_VOTE_YES) + 1);
            }
        }
    }
...
int Internal_GetClients(int[][] client_vote) {
    ...
    for (int i = 1; i <= MaxClients; i++) {
        if (g_ClientVotes[i] >= VOTE_PENDING) {
            ...
            client_vote[num_clients][VOTEINFO_CLIENT_ITEM]  = g_bPassed ? NATIVEVOTES_VOTE_YES : g_ClientVotes[i];
            num_clients++;
        }
    }
    return num_clients;
...
void Internal_Reset(bool cancel=false) {
    ...
    if (!cancel) {
        CreateTimer(5.0, Game_ResetVote, _, TIMER_FLAG_NO_MAPCHANGE);
    }
}
...
bool Internal_RedrawToClient(int client, bool revotes) {
    ...
    if (g_ClientVotes[client] >= 0) {
        ...
        g_Clients++;
        g_hVotes.Set(g_ClientVotes[client], g_hVotes.Get(g_ClientVotes[client]) - 1);
...
void CancelVoting() {
    if (g_bCancelled || g_hCurVote == null) {
        return;
    }
    g_bCancelled = true;
    EndVoting();
...
// native NativeVotes_Pass();
public int Native_Pass(Handle plugin, int numParams) {
    if (!Internal_IsVoteInProgress()) {
        return 0;
    }
    PassVoting();
...
public int Native_DisplayRawFail(Handle plugin, int numParams) {
    ...
    Game_DisplayRawVoteFail(clients, size, team);
    return 0;
}
...
public int Native_RegisterVoteCommand(Handle plugin, int numParams) {
    int overrideType = GetNativeCell(1);
    ...
    if (overrideType > NativeVotesOverride_Size) {
...
public int Native_DisplayCallVoteFail(Handle plugin, int numParams) {
    ...
    Game_DisplayCallVoteFail(client, reason);
    return 0;
}
