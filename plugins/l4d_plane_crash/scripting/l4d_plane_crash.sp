...
{
    g_hCvarAllow    = CreateConVar("l4d_plane_crash_allow",     "1",  "0=Plugin off, 1=Plugin on.");
    g_hCvarAngle    = CreateConVar("l4d_plane_crash_angle",     "1",  "0=Spawn the plane infront of you (crashes to the left), 1=Spawn so the plane crashes infront of you.");
    g_hCvarClear    = CreateConVar("l4d_plane_crash_clear",     "0",  "0=Off, Remove the plane crash this many seconds after the plane hits the ground.");
    g_hCvarDamage   = CreateConVar("l4d_plane_crash_damage",    "20", "0=Off, Other value will hurt players if they get crushed by some debris.");
    g_hCvarHorde    = CreateConVar("l4d_plane_crash_horde",     "24", "0=Off, Trigger a panic event this many seconds after the plane spawns.");
    g_hCvarModes    = CreateConVar("l4d_plane_crash_modes",     "",   "Turn on the plugin in these game modes, separate by commas (no spaces). (Empty = all).");
    g_hCvarModesOff = CreateConVar("l4d_plane_crash_modes_off", "",   "Turn off the plugin in these game modes, separate by commas (no spaces). (Empty = none).");
    g_hCvarModesTog = CreateConVar("l4d_plane_crash_modes_tog", "0",  "Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.");
    g_hCvarTime     = CreateConVar("l4d_plane_crash_time",      "0",  "0=Off, Otherwise creates a crash this many seconds after round start (triggers and custom map times override this cvar).");
