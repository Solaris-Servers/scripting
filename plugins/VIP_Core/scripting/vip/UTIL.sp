                        ...
                        const char[] szByWho = NULL_STRING)
{
    ...
    {
        FormatEx(SZF(szQuery), "INSERT INTO `vip_users` (`account_id`, `auth`, `sid`, `expires`, `group`, `name`, `lastvisit`) VALUES (%d, '%s', %d, %d, '%s', '%s', %d) \
        ON DUPLICATE KEY UPDATE `expires` = %d, `group` = '%s';", iAccountID, szSteamId, g_CVAR_iServerID, iExpires, szGroup, szName, iLastVisit, iExpires, szGroup);
    ...
    FormatEx(SZF(szQuery), "INSERT OR REPLACE INTO `vip_users` (`account_id`, `auth`, `name`, `expires`, `group`, `lastvisit`) VALUES (%d, '%s', '%s', %d, '%s', %d);", iAccountID, szSteamId, szName, iExpires, szGroup, iLastVisit);
    DBG_SQL_Query(szQuery)
    g_hDatabase.Query(SQL_Callback_OnVIPClientAdded, szQuery, hDataPack);
}
